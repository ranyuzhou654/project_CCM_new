...
def print_header():
    ...
    console.print(
        "🚀 CCM 全系统分析自动化脚本 v2.0", style="bold cyan", justify="center"
    )
    console.print("=" * 80, style="bold cyan")
    console.print()


def run_single_analysis_with_recovery(
    system, analysis_type, visualizer, num_trials, num_surrogates, results_dir
):
    """
    ...
    try:
        ...
        if "rossler" in system.lower() and (
            "numerical" in error_msg.lower()
            or "nan" in error_msg.lower()
            or "inf" in error_msg.lower()
        ):
...
def run_all_systems_analysis():
    ...
    console.print(
        f"🎯 将运行 {len(systems)} 个系统 × {len(analysis_types)} 种分析类型 ="
        f" {len(systems) * len(analysis_types)} 个分析任务"
    )
            ...
            for analysis_type in analysis_types:
                ...
                if success:
                    ...
                    if recovery_info and "恢复模式" in recovery_info:
                        ...
                        console.print(
                            f"    ✅ [green]{task_name} 完成[/green]"
                            f" [yellow]({recovery_info})[/yellow]"
                        )
                    else:
                    ...
                    console.print(
                        f"    ❌ [red]{task_name} 失败: {recovery_info[:50]}...[/red]"
                    )
    ...
    if rossler_recoveries > 0:
        console.print(
            "\n🔺 [yellow]Rössler系统恢复统计:"
            f" {rossler_recoveries} 个任务使用了恢复模式[/yellow]"
        )

    # 显示结果位置信息
    result_panel = Panel(
        f"📁 结果目录: [cyan]{results_manager.run_dir}[/cyan]\n📊 统计文件:"
        f" [cyan]{stats_file}[/cyan]\n📋 配置文件:"
        f" [cyan]{os.path.join(results_manager.run_dir, 'run_config.json')}[/cyan]",
        title="📂 结果文件位置",
        border_style="green",
    )
