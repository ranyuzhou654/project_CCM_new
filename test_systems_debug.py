...
def test_system_stability(system_name, plot=True):
    ...
    try:
        ...
        series = generate_time_series(
            system_name, num_systems, adjacency_matrix, 
            t_steps, epsilon, noise_level=0.0
        )
        ...
        if plot and not (has_nan or has_inf):
            ...
            plt.tight_layout()
            plt.savefig(f'debug_{system_name}_stability.png', dpi=150, bbox_inches='tight')
            plt.close()
...
def test_rossler_parameters():
    ...
    for dt in dt_values:
        ...
        def test_rossler_with_dt(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
            ...
            transient_sol = odeint(systems._rossler_ode, initial_state, transient_t, 
                                   args=(a, b, c, np.zeros_like(adjacency_matrix), 0, np.ones(3))).reshape((-1, 3, num_systems))
            ...
            try:
                solution = odeint(systems._rossler_ode, start_state, t, args=args, 
                                rtol=1e-6, atol=1e-8, mxstep=10000).reshape((-1, 3, num_systems))
                return solution[:, 0, :].T
            except Exception as e:
                print(f"   ❌ 积分失败: {e}")
...
if __name__ == "__main__":
    ...
    if unstable_systems:
        print(colored(f"\n发现 {len(unstable_systems)} 个不稳定系统需要修复!", "red", attrs=['bold']))
    else:
