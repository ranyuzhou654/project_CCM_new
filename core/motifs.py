...
def custom_transfer_entropy(source, target, k=1):
    ...
    h_y_future_y_history_x_history = _get_shannon_entropy(Counter(zip(y_future, y_history, x_history)))
...
def custom_conditional_transfer_entropy(source, target, condition, k=1):
    ...
    h_y_future_y_hist_z_hist = _get_shannon_entropy(Counter(zip(y_future, y_history, z_history)))
    ...
    h_y_future_y_x_z_hist = _get_shannon_entropy(Counter(zip(y_future, y_history, x_history, z_history)))
...
try:
    ...
    print(colored("警告: 'pyinform' 库加载失败。将使用纯Python实现的传递熵函数。", "yellow"))
    print(colored("分析速度会非常慢。强烈建议通过 'pip install --upgrade --force-reinstall pyinform' 修复。", "cyan"))
    print(colored("="*80, "yellow"))
...
def analyze_three_node_motifs_dual_method(system_type, visualizer, time_steps=2000, num_surrogates=200):
    """
    ...
    ccm_surrogate_methods = ['FFT', 'AAFT', 'IAAFT', 'Time Shift', 'Random Reorder', 'Conditional']
    ...
    for name, adjacency in tqdm(motifs.items(), desc="Analyzing Motifs"):
        ...
        time_series = generate_time_series(system_type, 3, adjacency, time_steps, epsilon)
        if np.any(~np.isfinite(time_series)): continue
        X, Z, Y = time_series[0], time_series[1], time_series[2]

        # --- Partial CCM 分析 ---
        partial_ccm_score = run_partial_ccm_trial(X, Y, Z, optimal_params['Dim'], optimal_params['tau'], num_surrogates)

        # --- CCM 分析 (使用所有代理方法) ---
        weights_Y, indices_Y = parameters(Y, optimal_params['Dim'], optimal_params['tau'])
        if weights_Y is None: continue
        ...
        for method in ccm_surrogate_methods:
            ...
            if method == 'Conditional':
                kwargs = {'conditional_series': Z, 'Dim': optimal_params['Dim'], 'tau': optimal_params['tau']}
            surrogates = generate_surrogates(X, method, num_surrogates, **kwargs)
            surrogate_coefs = np.abs(ccm_pearson(surrogates, weights_Y, indices_Y, shadow_len))
        ...
        X_d, Y_d, Z_d = _discretize_series(X), _discretize_series(Y), _discretize_series(Z)
        if X_d is not None and Y_d is not None and Z_d is not None:
            original_te_xy = transfer_entropy(X_d, Y_d, k=te_history_length)
            original_cte_xyz = conditional_transfer_entropy(X_d, Y_d, Z_d, k=te_history_length)
            ...
            for i in range(num_surrogates):
                ...
                if surr_X_d is not None:
                    ...
                    cte_dist.append(conditional_transfer_entropy(surr_X_d, Y_d, Z_d, k=te_history_length))
            
            te_pval = np.sum(np.array(te_dist) >= original_te_xy) / len(te_dist) if te_dist else 1.0
            cte_pval = np.sum(np.array(cte_dist) >= original_cte_xyz) / len(cte_dist) if cte_dist else 1.0
        else:
            print(colored(f"警告: 基序 {name} 的序列离散化失败，跳过TE分析。", "yellow"))
