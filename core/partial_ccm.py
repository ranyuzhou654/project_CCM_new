...
def _get_residuals(target_series, driver_series, dim, tau):
    ...
    driver_shadow = np.column_stack([
        driver_series[i * tau : L - (dim - 1 - i) * tau] for i in range(dim)
    ])
...
def run_partial_ccm_trial(X, Y, Z, dim, tau, num_surrogates=100):
    ...
    try:
        ...
        original_corr = np.abs(
            ccm_pearson(residual_X, weights_res_Y, indices_res_Y, shadow_len)
        )

        surrogates_res_X = generate_surrogates(residual_X, "Time Shift", num_surrogates)
        surrogate_corrs = np.abs(
            ccm_pearson(surrogates_res_X, weights_res_Y, indices_res_Y, shadow_len)
        )
