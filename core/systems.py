...
def generate_lorenz_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
    ...
    transient_sol = odeint(_lorenz_ode, initial_state, transient_t, args=(sigma, rho, beta, np.zeros_like(adjacency_matrix), 0, np.ones(3))).reshape((-1, 3, num_systems))
    # 为每个维度(x, y, z)计算标准差
    ...
    if dynamic_noise_level == 0.0:
        # 无动态噪声：一次性积分
        solution = odeint(_lorenz_ode, start_state, t, args=args, mxstep=5000).reshape((-1, 3, num_systems))
    else:
        # 有动态噪声：分步积分
        ...
        for i in range(t_steps):
            # 积分一小步
            sol_step = odeint(_lorenz_ode, current_state, [0, dt], args=args, mxstep=5000)
            # 添加噪声
    ...
    if noise_level > 0:
        ...
        solution += np.random.normal(scale=(noise_level * state_std[0]), size=solution.shape)
...
def generate_rossler_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
    ...
    transient_sol = odeint(_rossler_ode, initial_state, transient_t, args=(a, b, c, np.zeros_like(adjacency_matrix), 0, np.ones(3))).reshape((-1, 3, num_systems))
    state_std = np.std(transient_sol, axis=(0, 2))
    ...
    if epsilon != epsilon_safe:
        print(colored(f"警告: 耦合强度从 {epsilon} 限制到 {epsilon_safe} 以保证数值稳定性", "yellow"))
    ...
    if dynamic_noise_level == 0.0:
        ...
        try:
            if t_steps > 2000:  # 长序列使用分块积分
                ...
                for chunk in range(num_chunks):
                    ...
                    chunk_sol = odeint(_rossler_ode, current_state, 
                                     np.concatenate([[0], chunk_t[1:]]), args=args,
                                     rtol=1e-8, atol=1e-10, mxstep=15000)
                    
                    actual_len = min(len(chunk_sol) - 1, end_idx - start_idx)
                    solution[start_idx:start_idx + actual_len] = chunk_sol[1:actual_len+1].reshape((-1, 3, num_systems))
                    current_state = chunk_sol[-1]
            else:
                solution = odeint(_rossler_ode, start_state, t, args=args, 
                                 rtol=1e-8, atol=1e-10, mxstep=15000).reshape((-1, 3, num_systems))
        except Exception as e:
            print(colored(f"主积分失败，尝试备用方法: {str(e)}", "yellow"))
            # 备用方法：使用更小的步长和更严格的参数
            dt_backup = dt / 5
            t_backup = np.arange(0, t_steps * dt, dt_backup)
            solution_backup = odeint(_rossler_ode, start_state, t_backup, args=args,
                                   rtol=1e-6, atol=1e-8, mxstep=20000)
            # 重采样到目标时间点
        ...
        if dynamic_noise_level != dynamic_noise_safe:
            print(colored(f"警告: 动态噪声从 {dynamic_noise_level} 限制到 {dynamic_noise_safe} 以保证数值稳定性", "yellow"))
        
        noise_std = (dynamic_noise_safe * np.tile(state_std, num_systems)) * sqrt_dt
        
        for i in range(t_steps):
            try:
                sol_step = odeint(_rossler_ode, current_state, [0, dt], args=args, 
                                rtol=1e-8, atol=1e-10, mxstep=15000, 
                                h0=dt/1000)  # 指定初始步长
                noise = np.random.normal(scale=noise_std)
                current_state = sol_step[-1] + noise
                ...
                if np.any(np.abs(current_state) > 50):  # 更严格的界限
                    print(colored(f"警告: 在时间步 {i} 检测到大数值，重新初始化状态", "yellow"))
                    current_state = transient_sol[np.random.randint(len(transient_sol)//2, len(transient_sol))].flatten()
                    current_state += np.random.normal(scale=0.001, size=current_state.shape)
                ...
                if i > 10:
                    # 使用前几步的平均值
                    avg_state = np.mean([solution[j].flatten() for j in range(i-5, i)], axis=0)
                    current_state = avg_state + np.random.normal(scale=0.0001, size=current_state.shape)
                elif i > 0:
                    current_state = solution[i-1].flatten() + np.random.normal(scale=0.001, size=current_state.shape)
                else:
                    current_state = start_state + np.random.normal(scale=0.001, size=current_state.shape)
                solution[i] = current_state.reshape((3, num_systems))
...
# --- 逻辑斯蒂映射 (Logistic Map) ---
def generate_logistic_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
    r = 3.8
    # 预热以计算标准差
    transient_len = 1000
    ...
    for i in range(1, t_steps):
        ...
        interaction = epsilon_effective * (np.dot(adjacency_matrix.T, state_prev) - np.sum(adjacency_matrix, axis=0) * state_prev)
        next_state = r * state_prev * (1 - state_prev) + interaction
        if dynamic_noise_level > 0:
            # 使用计算出的标准差来缩放噪声
            next_state += np.random.normal(scale=dynamic_noise_level * series_std, size=next_state.shape)
        states[i] = np.clip(next_state, 0, 1)
        
    clean_series = states.T
    if noise_level > 0:
        clean_series += np.random.normal(scale=(noise_level * series_std), size=clean_series.shape)
        
    return clean_series

# --- 厄农映射 (Hénon Map) ---
def generate_henon_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
    a, b = 1.4, 0.3
    # 预热以计算标准差
    transient_len = 1000
    ...
    for t in range(1, t_steps):
        ...
        interaction = epsilon_effective * (np.dot(adjacency_matrix.T, x_prev) - np.sum(adjacency_matrix, axis=0) * x_prev)
        x_next = 1 - a * x_prev**2 + y_prev + interaction
        y_next = b * x_prev
        if dynamic_noise_level > 0:
            x_next += np.random.normal(scale=dynamic_noise_level * series_std, size=x_next.shape)
        if np.any(np.abs(x_next) > 10):
            x_next[np.abs(x_next) > 10] = np.random.rand() * 0.1
        states[t, :, 0] = x_next
        states[t, :, 1] = y_next
...
def _hindmarsh_rose_ode(state, t, a, b, c, d, r, s, x_R, I, adjacency_matrix, epsilon, state_std):
    num_systems = adjacency_matrix.shape[0]
    x, y, z = state.reshape((3, num_systems))
...
def generate_hindmarsh_rose_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0, dynamic_noise_level=0.0):
    ...
    transient_sol = odeint(_hindmarsh_rose_ode, initial_state, transient_t, args=(a, b, c, d, r, s, x_R, I, np.zeros_like(adjacency_matrix), 0, np.ones(3))).reshape((-1, 3, num_systems))
    state_std = np.std(transient_sol, axis=(0, 2))
    ...
    if dynamic_noise_level == 0.0:
        solution = odeint(_hindmarsh_rose_ode, start_state, t, args=args, mxstep=10000).reshape((-1, 3, num_systems))
    else:
        solution = np.zeros((t_steps, 3, num_systems))
        current_state = start_state
        sqrt_dt = np.sqrt(dt)
        ...
        for i in range(t_steps):
            sol_step = odeint(_hindmarsh_rose_ode, current_state, [0, dt], args=args, mxstep=10000)
            noise = np.random.normal(scale=noise_std)
            current_state = sol_step[-1] + noise
            solution[i] = current_state.reshape((3, num_systems))
...
def _kuramoto_ode(theta, t, natural_freqs, adjacency_matrix, epsilon):
    ...
    for i in range(num_systems):
        coupling_sum = sum(np.sin(theta[j] - theta[i]) for j in range(num_systems) if adjacency_matrix[j, i] == 1)
        dtheta_dt[i] = natural_freqs[i] + (epsilon / num_systems) * coupling_sum
    return dtheta_dt

def generate_kuramoto_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0):
    ...
    phases = odeint(_kuramoto_ode, initial_phases, t, args=(natural_freqs, adjacency_matrix, epsilon), mxstep=5000)
...
# --- Mackey-Glass 时延系统 ---
def generate_mackey_glass_series(num_systems, adjacency_matrix, t_steps, epsilon, noise_level=0.0):
    ...
    for t in range(history_len, total_len - 1):
        for i in range(num_systems):
            ...
            interaction = sum(epsilon_effective * (series[j, t] - series[i, t]) for j in range(num_systems) if adjacency_matrix[j, i] == 1)
...
def generate_time_series(system_type, num_systems, adjacency_matrix, t_steps, epsilon, **kwargs):
    """
    根据系统类型分发任务的工厂函数。
    """
    try:
        ...
        system_map = {
            ...
            'lorenz_dynamic_noise': (generate_lorenz_series, {'dynamic_noise_level': noise_level}),
            'rossler': (generate_rossler_series, {}),
            'noisy_rossler': (generate_rossler_series, {'noise_level': noise_level}),
            'rossler_dynamic_noise': (generate_rossler_series, {'dynamic_noise_level': noise_level}),
            'logistic': (generate_logistic_series, {}),
            'noisy_logistic': (generate_logistic_series, {'noise_level': noise_level}),
            'logistic_dynamic_noise': (generate_logistic_series, {'dynamic_noise_level': noise_level}),
            'henon': (generate_henon_series, {}),
            'noisy_henon': (generate_henon_series, {'noise_level': noise_level}),
            'henon_dynamic_noise': (generate_henon_series, {'dynamic_noise_level': noise_level}),
            'hindmarsh_rose': (generate_hindmarsh_rose_series, {}),
            'noisy_hindmarsh_rose': (generate_hindmarsh_rose_series, {'noise_level': noise_level}),
            'hindmarsh_rose_dynamic_noise': (generate_hindmarsh_rose_series, {'dynamic_noise_level': noise_level}),
            ...
            'noisy_mackey_glass': (generate_mackey_glass_series, {'noise_level': noise_level}),
        }
