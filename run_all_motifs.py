...
def run_single_motifs_with_recovery(
    system, visualizer, time_steps, num_surrogates, results_dir
):
    """
...
def run_all_systems_motifs():
    ...
    rossler_systems = ["rossler", "noisy_rossler", "rossler_dynamic_noise"]
    ...
    console.print(
        f"📊 每个系统: 时间序列长度 {time_steps}, {num_surrogates} 个代理数据"
    )
    ...
    with Progress() as progress:
        ...
        for system in systems:
            ...
            if success:
                ...
                console.print(
                    f"  ❌ [red]{system} 基序分析失败: {recovery_info[:50]}...[/red]"
                )
    ...
    if rossler_recoveries > 0:
        console.print(
            ...
            f" {rossler_recoveries} 个系统使用了恢复模式[/yellow]"
        )
