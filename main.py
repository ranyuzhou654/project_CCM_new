...
# 导入项目模块
try:
    from core.analysis import run_full_analysis, run_enhanced_analysis
...
def run_analysis_command(args):
    """处理 'run-analysis' 命令"""
    console.print(
        "🚀 [bold green]开始运行: [/bold green]"
        f" [yellow]{args.system.capitalize()}[/yellow] 系统上的"
        f" [yellow]{args.analysis_type}[/yellow] 分析..."
    )
...
def run_motifs_command(args):
    ...
    console.print(
        "🚀 [bold green]开始运行: [/bold green]"
        f" [yellow]{args.system.capitalize()}[/yellow] 系统上的三节点基序分析 (CCM +"
        " CTE 双核)..."
    )
...
def run_enhanced_analysis_command(args):
    """处理 'run-enhanced-analysis' 命令"""
    console.print(
        "🚀 [bold green]开始运行增强版分析: [/bold green]"
        f" [yellow]{args.system.capitalize()}[/yellow] 系统上的"
        f" [yellow]{args.analysis_type}[/yellow] 分析..."
    )
    console.print(f"📊 [cyan]配置信息:[/cyan]")
    console.print(f"   - 置信度方法: {args.confidence_method.upper()}")
    console.print(f"   - 自适应测试: {'禁用' if args.no_adaptive else '启用'}")
    console.print(f"   - Bootstrap CI: {'禁用' if args.no_bootstrap else '启用'}")
    console.print(f"   - 代理数量: {args.surrogates}")
    console.print(f"   - 试验次数: {args.trials}")
    console.print(f"   - 并行进程: {args.workers if args.workers else '自动'}")
...
def optimize_params_command(args):
    ...
    console.print(
        f"🚀 [bold green]开始为 {args.system.capitalize()} 系统优化嵌入参数...[/bold"
        " green]"
    )
    console.print(f"将生成长度为 {args.length} 的测试序列进行分析。")

    best_params = optimize_embedding_for_system(
        system_type=args.system, series_length=args.length
    )
...
def main():
    ...
    parser_analysis.add_argument(
        ...
        type=str,
        required=True,
        choices=["length", "degree", "coupling", "nodes", "noise"],
        help="要执行的分析类型。",
    )
    parser_analysis.add_argument(
        "--trials", "-t", type=int, default=20, help="每次参数设置的试验次数。"
    )
    parser_analysis.add_argument(
        "--surrogates", "-n", type=int, default=100, help="生成的代理数据数量。"
    )
    parser_analysis.add_argument(
        "--workers", "-w", type=int, default=None, help="并行进程数（默认自动）。"
    )
    parser_analysis.set_defaults(func=run_analysis_command)

    # --- 'run-enhanced-analysis' 子命令 ---
    parser_enhanced = subparsers.add_parser(
        "run-enhanced-analysis", help="运行增强版多维度性能分析（包含改进的统计方法）。"
    )
    parser_enhanced.add_argument(
        "--system",
        "-s",
        type=str,
        required=True,
    ...
    parser_enhanced.add_argument(
        "--surrogates", "-n", type=int, default=200, help="生成的代理数据数量。"
    )
    parser_enhanced.add_argument(
        "--confidence-method",
        "-c",
    ...
    parser_enhanced.add_argument(
        "--no-adaptive", action="store_true", help="禁用自适应代理数量测试。"
    )
    parser_enhanced.add_argument(
        "--no-bootstrap", action="store_true", help="禁用Bootstrap置信区间计算。"
    )
    parser_enhanced.add_argument(
        "--workers", "-w", type=int, default=None, help="并行进程数（默认自动）。"
    )
    parser_enhanced.set_defaults(func=run_enhanced_analysis_command)

    # --- 'run-motifs' 子命令 ---
    parser_motifs = subparsers.add_parser(
        "run-motifs", help="运行三节点基序因果分析 (CCM+CTE)。"
    )
    parser_motifs.add_argument(
        "--system",
        "-s",
        type=str,
        required=True,
    ...
    parser_motifs.add_argument(
        "--length", "-l", type=int, default=2000, help="用于分析的时间序列长度。"
    )
    parser_motifs.add_argument(
        "--surrogates", "-n", type=int, default=200, help="生成的代理数据数量。"
    )
    ...
    parser_optimize = subparsers.add_parser(
        "optimize-params", help="自动寻找给定系统的最佳嵌入参数 (tau, Dim)。"
    )
        ...
        if args.command == "run-analysis" and args.analysis_type == "noise":
            ...
            if "noisy_" not in system_name and "_dynamic_noise" not in system_name:
                ...
                if observational_version in available_systems:
                    ...
                    console.print(
                        f"[yellow]提示: 已自动选择观测噪声版本 '{args.system}'"
                        " 进行噪声分析。[/yellow]"
                    )
                elif dynamic_version in available_systems:
                    args.system = dynamic_version
                    console.print(
                        f"[yellow]提示: 已自动选择动态噪声版本 '{args.system}'"
                        " 进行噪声分析。[/yellow]"
                    )
                else:
                    console.print(
                        f"[bold red]错误: 系统 '{clean_name}' 找不到可用的噪声版本。[/bold red]"
                    )
                    sys.exit(1)
