...
def stress_test_system(system_name, test_conditions):
    ...
    for condition_name, params in test_conditions.items():
        ...
        try:
            ...
            max_abs_val = np.max(np.abs(series)) if not (has_nan or has_inf) else float('inf')
...
def test_all_systems_stress():
    """所有系统的压力测试"""
    systems = ['lorenz', 'rossler', 'logistic', 'henon', 'hindmarsh_rose', 'kuramoto', 'mackey_glass']
    ...
    for system in dynamic_systems:
        ...
        try:
            ...
            for noise_level in noise_levels:
                series = generate_time_series(
                    system, num_systems, adjacency_matrix, 
                    t_steps, epsilon, noise_level=noise_level
                )
...
def main():
    ...
    if total_failures == 0:
        ...
        print(colored(f"⚠️  发现 {total_failures} 个问题需要修复", "yellow", attrs=['bold']))
